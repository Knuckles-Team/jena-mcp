...
|----------|----------|-------|
| `APACHE_JENA_URL` | ✅ | Fuseki base URL (aliases `JENA_FUSEKI_URL`, `JENA_URL`); defaults to `[configured-endpoint]` |
...
```
action=query dataset=ds
sparql="PREFIX : <[configured-endpoint]> SELECT ?s WHERE { ?s a :JenaDataset } LIMIT 10"
```
...
sparql="PREFIX : <[configured-endpoint]> INSERT DATA { :ds1 a :JenaDataset ; :datasetName \"ds\" }"
```
